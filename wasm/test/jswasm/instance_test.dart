...
void main() {
  group('Empty', () {
    ...
    test(
      'From module async',
      () async => checkInstance(await Instance.fromModuleAsync(module), module),
    );
    test('Invalid source throws CompileError', () {
      final invalidBytes =
          Uint8List.fromList([0, 0x61, 0x73, 0x6d, 0, 0, 0, 0]);
      expect(
        () => Instance.fromBytesAsync(invalidBytes),
        throwsA(isA<CompileError>()),
      );
      expect(
        () => Instance.fromBufferAsync(invalidBytes.buffer),
        throwsA(isA<CompileError>()),
      );
    });
    test('RuntimeError on instantiation', () {
      ...
      final moduleBytes = Uint8List.fromList(
        '\x00\x61\x73\x6D\x01\x00\x00\x00\x01\x04\x01\x60\x00\x00\x03'
                '\x02\x01\x00\x08\x01\x00\x0A\x05\x01\x03\x00\x00\x0B'
            .codeUnits,
      );
      final module = Module.fromBytes(moduleBytes);
      expect(() => Instance.fromModule(module), throwsA(isA<RuntimeError>()));
      expect(
        () => Instance.fromModuleAsync(module),
        throwsA(isA<RuntimeError>()),
      );
      expect(
        () => Instance.fromBytesAsync(moduleBytes),
        throwsA(isA<RuntimeError>()),
      );
      expect(
  ...
  group('Imports', () {
    ...
    test('Instantiate with map', () {
      ...
      expect(
        Instance.fromModule(module, importMap: importMap),
        isA<Instance>(),
      );
      expect(
        Instance.fromModuleAsync(module, importMap: importMap),
        completion(isA<Instance>()),
      );
      expect(
        Instance.fromBytesAsync(moduleBytes, importMap: importMap),
        completion(isA<Instance>()),
      );
      expect(
        Instance.fromBufferAsync(moduleBytes.buffer, importMap: importMap),
        completion(isA<Instance>()),
      );
    });
    test('Instantiate with object', () {
      expect(
        Instance.fromModule(module, importObject: importObject),
        isA<Instance>(),
      );
      expect(
        Instance.fromModuleAsync(module, importObject: importObject),
        completion(isA<Instance>()),
      );
      expect(
        Instance.fromBytesAsync(moduleBytes, importObject: importObject),
        completion(isA<Instance>()),
      );
      expect(
  ...
  group('Run', () {
    test('Memory access', () {
      ...
      final moduleBytes = Uint8List.fromList(
        '\x00\x61\x73\x6D\x01\x00\x00\x00\x01\x04\x01\x60\x00\x00\x02\x0A\x01'
            '\x03\x65\x6E\x76\x01\x6D\x02\x00\x01\x03\x02\x01\x00\x07\x09\x02'
            '\x01\x66\x00\x00\x01\x6D\x02\x00\x0A\x11\x01\x0F\x00\x41\x04\x41'
            '\x04\x28\x02\x00\x41\x01\x6A\x36\x02\x00\x0B'
            .codeUnits,
      );
      final memory = Memory(initial: 1);
      final memoryView = memory.buffer.asUint32List();
      memoryView[1] = 42;
      final instance = Instance.fromModule(
        Module.fromBytes(moduleBytes),
        importMap: {
          'env': {'m': memory}
        },
      );
    ...
    test('Globals', () {
      ...
      void checkValues(Instance instance) {
        ...
        expect(
          JsBigInt.toBigInt(functions['g_i64']!() as Object),
          BigInt.from(3),
        );
  ...
  test(
    ...
    () {
      ...
      expect(
          instance.functions['select']!('string', 'longString'), 'longString',);
    },
    testOn: 'firefox',
  );
  test('Tables', () {
