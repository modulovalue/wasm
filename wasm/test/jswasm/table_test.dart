...
void main() {
  ...
  test(
    'Create with maximum and grow - externref',
    () {
  ...
  test(
    'Create table with maximum and grow beyond - externref',
    () {
  ...
  assert(
    () {
      test('Create table with invalid size', () {
        expect(
          () => Table.funcref(initial: -1),
          throwsA(isA<AssertionError>()),
        );
        expect(
          () => Table.funcref(initial: 42, maximum: 1),
          throwsA(isA<AssertionError>()),
        );
      });
      test(
        'Create table with invalid size - externref',
        () {
          expect(
            () => Table.externref(initial: -1),
            throwsA(isA<AssertionError>()),
          );
          expect(
            () => Table.externref(initial: 42, maximum: 1),
            throwsA(isA<AssertionError>()),
          );
        },
        testOn: 'firefox',
